...
| 🚧 | **T1S link** — marvin (PLCA coordinator) ↔ `guitar` actuator node (follower) **working** over 10BASE-T1S (LAN8651 each end): command TX + presence heartbeat + link/`nodes` diagnostics. Remaining: move `fretboard` to a T1S detector node, multi-node active selection, and dumb PoDL (power on the pair). [Detail](docs/t1s-podl-link.md) |
