...
>
> This document describes the original PS2 / Elgato / PC-OpenCV architecture from project inception (2026-01-28). The system has since been re-architected around three live subprojects, each with its own spec:
>
> - **[`firmware/marvin/docs/spec.md`](firmware/marvin/docs/spec.md)** — SAM9X75 host: HDMI capture, reference CV detection, timing pipeline, operator UI, reference-data recording.
> - **[`firmware/fretboard/SPEC.md`](firmware/fretboard/SPEC.md)** — PIC32CM6408 actuator MCU: phototransistor sensors and open-drain GPIO output to a Wii guitar controller.
> - **[`tools/fret-tuner/SPEC.md`](tools/fret-tuner/SPEC.md)** — off-band Python/FastAPI calibration and detector-tuning tool.
>
> Read the subproject specs for current behavior. The body below is kept for context on early-stage decisions and constraints; do not treat it as authoritative.
