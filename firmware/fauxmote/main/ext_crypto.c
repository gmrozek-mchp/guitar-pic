#include "ext_crypto.h"

/* Wii extension cipher (1st-party extensions: nunchuk/classic/guitar). The
 * algorithm and constant tables were reverse-engineered by Hector Martin
 * ("marcan"); the tables here are cross-checked against the Dolphin emulator.
 * See firmware/fauxmote/docs/journal.md for provenance and the GH3 key handshake.
 *
 * The host writes a 16-byte key to extension register 0x40; both sides derive ft/sb
 * from it. The extension (us) sends data through Encrypt(); the host runs the
 * inverse. Only addr % 8 of each byte's register address matters. */

/* keygen S-box, used only to derive the table-selection index. */
static const uint8_t k_keygen[256] = {
    ,
};

/* The eight S-boxes; idx (0..6) selects the pair (k_sbox[idx], k_sbox[idx+1]). */
static const uint8_t k_sbox[8][256] = {
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
  {
    ,
  },
};

static const uint8_t k_ans[7][6] = {
    { 0xa8, 0x77, 0xa6, 0xe0, 0xf7, 0x43 },
    { 0x5a, 0x35, 0x85, 0xe2, 0x72, 0x97 },
    { 0x8f, 0xb7, 0x1a, 0x62, 0x87, 0x38 },
    { 0x0d, 0x67, 0xc7, 0xbe, 0x4f, 0x3e },
    { 0x20, 0x76, 0x37, 0x8f, 0x68, 0xb7 },
    { 0xa9, 0x26, 0x3f, 0x2b, 0x10, 0xe3 },
    { 0x30, 0x7e, 0x90, 0x0e, 0x85, 0x0a },
};

static inline uint8_t ror8(uint8_t v, uint8_t n)
{
    n &= 7;
    return (uint8_t)((v >> n) | (v << (8 - n)));
}

void ExtCrypto_GenTables(ext_crypto_t *st, const uint8_t key[16])
{
    uint8_t t0[10];
    for (int i = 0; i < 10; i++) {
        t0[i] = k_keygen[key[9 - i]];
    }

    /* idx is not transmitted; brute-force the one the host used (0..6). */
    int idx = 0;
    for (; idx < 7; idx++) {
        const uint8_t *a = k_ans[idx];
        if (key[0xf] == (uint8_t)((ror8(a[0] ^ t0[5], t0[2]) - t0[9]) ^ t0[4]) &&
            key[0xe] == (uint8_t)((ror8(a[1] ^ t0[1], t0[0]) - t0[5]) ^ t0[7]) &&
            key[0xd] == (uint8_t)((ror8(a[2] ^ t0[6], t0[8]) - t0[2]) ^ t0[0]) &&
            key[0xc] == (uint8_t)((ror8(a[3] ^ t0[4], t0[7]) - t0[3]) ^ t0[2]) &&
            key[0xb] == (uint8_t)((ror8(a[4] ^ t0[1], t0[6]) - t0[3]) ^ t0[4]) &&
            key[0xa] == (uint8_t)((ror8(a[5] ^ t0[7], t0[8]) - t0[5]) ^ t0[9])) {
            break;
        }
    }
    if (idx > 6) idx = 6;   /* no match (e.g. all-zero homebrew key); fall back */

    const uint8_t *sa = k_sbox[idx];
    const uint8_t *sc = k_sbox[idx + 1];

    st->ft[0] = sa[key[0xb]] ^ sc[key[0x6]];
    st->ft[1] = sa[key[0xd]] ^ sc[key[0x4]];
    st->ft[2] = sa[key[0xa]] ^ sc[key[0x2]];
    st->ft[3] = sa[key[0xf]] ^ sc[key[0x7]];
    st->ft[4] = sa[key[0xe]] ^ sc[key[0x5]];
    st->ft[5] = sa[key[0xc]] ^ sc[key[0x0]];
    st->ft[6] = sa[key[0x9]] ^ sc[key[0x3]];
    st->ft[7] = sa[key[0x8]] ^ sc[key[0x1]];

    st->sb[0] = sa[key[0xf]] ^ sc[key[0x8]];
    st->sb[1] = sa[key[0xa]] ^ sc[key[0x5]];
    st->sb[2] = sa[key[0xc]] ^ sc[key[0x9]];
    st->sb[3] = sa[key[0xd]] ^ sc[key[0x0]];
    st->sb[4] = sa[key[0xb]] ^ sc[key[0x2]];
    st->sb[5] = sa[key[0xe]] ^ sc[key[0x1]];
    st->sb[6] = sa[key[0x6]] ^ sc[key[0x4]];
    st->sb[7] = sa[key[0x7]] ^ sc[key[0x3]];
}

void ExtCrypto_Encrypt(const ext_crypto_t *st, uint8_t *buf, int addr, int len)
{
    for (int i = 0; i < len; i++) {
        int k = (i + addr) & 7;
        buf[i] = (uint8_t)((buf[i] - st->ft[k]) ^ st->sb[k]);
    }
}
