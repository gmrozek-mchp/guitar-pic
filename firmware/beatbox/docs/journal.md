...
- [~] **B1 — Data model / producer.** Lightshow half landed: `config.mcc/src/publish.{c,h}` maps each
      ...
      choreography layer), formally retiring the parked `servo`/`nod_engine`/`ws2812`.
- [~] **B4 — Live show (beatbox side).** Beat-frame **transmit** landed: `T1SFollower_SendBeatFrame`
      broadcasts the serialized `LightshowFrame` under ethertype `0x88B8` at the ~23.4 Hz frame rate;
      `main.c` drains `Publish_HasLightshowFrame` into it, `t1s` CLI shows a `beat tx` counter. Pending:
      the **lightshow consumer** (accept `0x88B8`, drive LEDs), then the lemmy position-command channel
      (`0x88B9` unicast) once its payload + choreography exist.

## Open questions

- **lemmy command payload.** What does beatbox send lemmy — absolute neck/jaw angles, or a
  higher-level pose/energy the puppet interprets? (Transport is decided: unicast to `02:..:06`,
  ethertype `0x88B9` — see decision log; the *payload semantics* are what's open.) Shared design
  with lemmy, which drops its own beat-driven nod logic to become a position follower. Resolve at B1/B2.
...
|------|----------|-----------|
| 2026-07-31 | **Board LEDs LED0..LED7 (RC8..RC15) driven as an audio VU meter.** New `config.mcc/src/vu_meter.{c,h}`: `VU_Update(&f)` maps each `BeatFrame`'s `raw_env` (0-10000 absolute envelope) to 0-8 lit segments, filling the bar from **LED7 (bottom) up to LED0 (top)** per the board layout (LED7 physically leftmost). Instant attack, linear per-frame decay (`VU_DECAY` 700/frame ≈ 0.36 s full-scale fall) so peaks hold and drop smoothly; `VU_FULL_SCALE` 6000 (loud line-in peaks ~4000-5000, leaving headroom) is the single tuning knob. Drives via the MCC `LEDn_SetHigh/SetLow` pin macros (not raw `LATC`) so a pin-manager remap can't break it. Wired into `main.c` (`VU_Initialize()` at boot, `VU_Update(&f)` in the `Beat_HasFrame()` block alongside `beat_indicator`/`Publish_Update`); no CLI command (embedded-cli is already at its 8-binding static max). | The 8 discrete board LEDs were sitting unused; a VU meter is the obvious at-a-glance level display and reuses the envelope already in `BeatFrame`. Absolute `raw_env` (not the auto-ranged 0-1000 envelope) keeps it an honest loudness meter — quiet passages read low. Attack/decay smoothing suits the ~23.4 Hz frame rate (a raw per-frame peak would flicker). Macro-based LED drive matches how the rest of the app touches GPIO and stays correct if the pins move. |
...
### 2026-07-31 — Board LEDs as a VU meter

- **Added `config.mcc/src/vu_meter.{c,h}`.** The 8 dev-board LEDs (LED0..LED7 = RC8..RC15) now show
  an audio level bar filling from **LED7 (bottom/left) up to LED0 (top)**. `VU_Update(&f)` runs each
  new `BeatFrame`, mapping `raw_env` (0-10000) → 0-8 lit segments with instant attack + linear decay
  (`VU_DECAY` 700/frame); `VU_FULL_SCALE` 6000 is the tuning knob. Drives through the MCC
  `LEDn_SetHigh/SetLow` macros. See decision log.
- **Wiring:** `main.c` — `VU_Initialize()` before `CLI_Initialize()`, `VU_Update(&f)` in the
  `Beat_HasFrame()` block; `vu_meter.c` added to `cmake/beatbox/default/user.cmake`. No CLI command
  (embedded-cli is at its 8-binding max). Pins were already outputs (`TRISC=0x00FF`, `LATC=0`).
- **Not yet built or on hardware** (user builds/flashes). Bench check: LED bar tracks loudness,
  peaks hold briefly then fall; tune `VU_FULL_SCALE`/`VU_DECAY` to taste against live audio.
...
- **Transmit landed.** `T1SFollower_SendBeatFrame(payload, len)` broadcasts the beat frame:
  `[FF:FF:FF:FF:FF:FF][src 02:..:05][ethertype 0x88B8][payload]`, built into a dedicated
  `s_beat_frame` staging buffer with its own `s_beat_busy`/`beat_tx_done` (independent of the
  heartbeat's, so a beat frame and a heartbeat can be in flight together). Sends via
  `TC6_SendRawEthernetPacket`; returns false (frame dropped) when PLCA isn't operating or a prior
  beat frame is still draining.
- **Payload serialization** moved to the producer: `Publish_SerializeLightshow(&lf, out)` writes the
  8 `LightshowFrame` bytes in field order, so the on-wire layout is owned by `publish` rather than
  relying on struct packing. `LIGHTSHOW_FRAME_LEN` (8) exported for the caller's buffer.
- **Glue:** `main.c` drains the consume-once `Publish_HasLightshowFrame()` each loop iteration —
  snapshot, serialize, `T1SFollower_SendBeatFrame` — right before `T1SFollower_Tasks()`. `t1s` CLI
  now prints a `beat tx` frame counter next to rx/errors.
- **Scope:** beatbox side only, as asked; wire format designed so the **lightshow consumer** (accept
  `0x88B8`, decode the 8 bytes, drive LEDs) drops in next. Followers already pass broadcast at the
  MAC filter, so no other node needs a change to tolerate the new traffic (see the marvin journal).
  `tempo`/`phase` stay 0 until the tempo/BPM+phase layer.
