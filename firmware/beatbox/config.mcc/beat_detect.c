...
// =============================================================================
// Audio Envelope + Spectral Flux feature extraction (~23.4 Hz per FFT frame):
//   1. Audio Envelope: peak absolute sample value in this frame
//   2. Spectral Flux: sum of positive magnitude changes (all bins)
//
// Features are streamed to the host and consumed by the beat-decision logic in
// main.c. Audio pass-through is handled in the ADC ISR.
// =============================================================================
...
// Sample counter (48 kHz tick, for timing measurement)
static volatile uint32_t sample_counter = 0;
static volatile uint32_t frame_sample_stamp = 0;  // counter at last frame
static volatile uint16_t out_frame_us = 0;         // measured frame period in 10us units
...
void BeatDetect_RunFFT(void)
{
    ...
    // --- Frame timing (in 10us units) ---
    uint32_t now = sample_counter;
    uint32_t delta_samples = now - frame_sample_stamp;
    frame_sample_stamp = now;
    // 48000 samples/sec → 1 sample = 20.833us → delta * 20.833 / 10 = delta * 2.0833
    // Approximate: (delta * 2) + (delta / 12)  ≈  delta * 2.083
    out_frame_us = (uint16_t)((delta_samples * 2) + (delta_samples / 12));
...
uint16_t BeatDetect_GetFrameTime(void) { return out_frame_us; }
...
// Stubs (keep build happy)
bool BeatDetect_HasBeat(void) { return false; }
float BeatDetect_GetStrength(void) { return 0.0f; }
float BeatDetect_GetFlux(void) { return (float)out_flux; }
float BeatDetect_GetFluxAvg(void) { return 0.0f; }
uint8_t BeatDetect_GetActiveBand(void) { return 0; }
float BeatDetect_GetBPM(void) { return 0.0f; }
float BeatDetect_GetConfidence(void) { return 0.0f; }
bool BeatDetect_IsMusicPresent(void) { return out_envelope > 50; }
bool BeatDetect_HasSpectrum(void) { return false; }
void BeatDetect_GetSpectrum(uint8_t *out) { memset(out, 0, BEAT_SPECTRUM_BINS); }
void BeatDetect_GetDrumSignal(uint8_t *out) { memset(out, 0, BEAT_DRUM_HIST_LEN); }
void BeatDetect_SetThreshold(float value) { (void)value; }
float BeatDetect_GetThreshold(void) { return 0.0f; }
void BeatDetect_SetBandMask(uint8_t mask) { (void)mask; }
uint8_t BeatDetect_GetBandMask(void) { return 0; }
void BeatDetect_SetMode(uint8_t mode) { (void)mode; }
uint8_t BeatDetect_GetMode(void) { return 0; }
