...
static volatile uint32_t s_tx_count;     /* frames this node has completed sending (HB + beat) */
                       ...
                       void *pTag, void *pGlobalTag)
{
    ...
    (void)pGlobalTag;
    s_hb_busy = false;
    s_tx_count++;   /* count completed transmits */
}
                         ...
                         void *pTag, void *pGlobalTag)
{
    ...
    (void)pGlobalTag;
    s_beat_busy = false;
    s_tx_count++;   /* count completed transmits (beat frames + heartbeats) */
}
                               ...
                               uint64_t *rxTimestamp, void *pGlobalTag)
{
    ...
    /* beatbox drives no bus actuator; keep the first payload byte for diagnostics.
     * The lemmy/lightshow publish path lands at B4. */
