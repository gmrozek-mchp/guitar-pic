...
leResult screenShow_Marvin(void)
{
    ...
    // layer 10
    root10 = leWidget_New();
    root10->fn->setSize(root10, 1060, 560);
    root10->fn->setBackgroundType(root10, LE_WIDGET_BACKGROUND_NONE);
    root10->fn->setMargins(root10, 0, 0, 0, 0);
    root10->flags |= LE_WIDGET_IGNOREEVENTS;
    root10->flags |= LE_WIDGET_IGNOREPICK;

    Marvin_PANEL_ROLE = leWidget_New();
    Marvin_PANEL_ROLE->fn->setPosition(Marvin_PANEL_ROLE, 0, 0);
    Marvin_PANEL_ROLE->fn->setSize(Marvin_PANEL_ROLE, 1060, 560);
    Marvin_PANEL_ROLE->fn->setScheme(Marvin_PANEL_ROLE, &SCHEME_FILL_ZINC_900);
    root10->fn->addChild(root10, (leWidget*)Marvin_PANEL_ROLE);

    leAddRootWidget(root10, 10);
    leSetLayerColorMode(10, LE_COLOR_MODE_RGB_565);
...
void screenUpdate_Marvin(void)
{
    ...
    root10->fn->setSize(root10, root10->rect.width, root10->rect.height);
}
...
void screenHide_Marvin(void)
{
    ...
    leRemoveRootWidget(root10, 10);
    leWidget_Delete(root10);
    root10 = NULL;

    Marvin_PANEL_ROLE = NULL;
