...
- **The only remaining problem is marvin-side:** it parks in `vApplicationMallocFailedHook` because its **FreeRTOS heap is fully exhausted at startup** — `configTOTAL_HEAP_SIZE = 40960` (FreeRTOSConfig.h:271), `ucHeap`@0x24091288 (40 KB, in DDR), and at the fault `xNextFreeByte = 0x9ff0` (40,944/40,960 used). Sane value ⇒ not a `.bss`-clear bug; it's real exhaustion. Fix is marvin-side: **raise `configTOTAL_HEAP_SIZE`** (or investigate what consumes ~40 KB at boot). Likely affects normal SD boot too unless the SD build differs. **Infrastructure (DDR init + load + boot via JTAG, all macOS/OpenOCD) is DONE.**
