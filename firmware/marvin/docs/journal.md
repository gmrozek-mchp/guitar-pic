...
- **MCC-file modifications maintenance risk.** A from-scratch MCC regen on 2026-05-15 confirmed **four** local modifications get clobbered. The 2026-05-20 USB-host regen added a fifth. Re-apply after every regen:
  ...
  Recovery plan: re-apply all ten (small, self-contained diffs). Long-term options are (a) file MCC bugs, (b) shim into our own files, (c) live with periodic re-application.
...
- **`load-ram.sh`** — single OpenOCD session: `marvin_load_ram` (`reset init` → at91bootstrap → load + run marvin). Repeatable, no power-cycle. `reset` / `reset halt` / `reset init` now also work in plain debug sessions (resets a running marvin, like the board button).
...
- **Production path clarified.** The `sam9x75_curiosity_pro{nf,df_qspi,sd}_uboot_defconfig` media configs are NOT u-boot-specific — they load the next-stage binary from flash into DRAM (`0x23f00000`, where marvin runs) and jump. For bare-metal marvin, flash `harmony.bin` as that second stage (no actual u-boot). Build the matching medium when doing standalone boot (Phase 2).
...
- **Bootable microSD — `openocd/make-sdcard.sh`.** macOS-native: FAT-format the card (MBR) and write `binaries/boot.bin` + `out/harmony.bin` to root (SAM9X75 ROM SD-boot layout, matching the SAM-BA `qspi_flash.bat` naming). Refuses a fixed internal disk and requires retyping the disk id before erasing. Not yet exercised on a real card.
