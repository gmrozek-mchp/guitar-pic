...
**2026-08-09 — the output enable is now echoed in the heartbeat, closing the loop on `0x88B9` op `0x01`.** One line: `0x88B6` `flags` bit1 = `BeatShow_IsEnabled()` (bit0 = synced was the only user; no length or version change). lightshow's own gate needed **no change** — it was already exactly the right one, and of the three actuator nodes it was the only one that didn't need work when marvin's dashboard ACTUATORS toggles were wired up (guitar had no control channel at all; lemmy's `NOD_EN` only gated the beat-driven neck). What the echo buys: `0x88B9` has no ACK and a follower boots on its own default, so marvin previously had no way to know whether a command landed. Now it compares the report against what it last commanded and re-pushes on mismatch — which corrects a lost frame, a node reboot, or a local `show` change, and lets marvin's dashboard dot mean "the LEDs are enabled" rather than "we sent a frame". **Note the ownership shift:** `s_enabled` still defaults on locally, but marvin's own default (also on) is now pushed and reconciled, so the coordinator is authoritative whenever it's on the bus. Wire contract: [`docs/t1s-podl-link.md`](../../../docs/t1s-podl-link.md) §7.1/§7.2. Builds clean; additive on the wire → standalone reflash. **Pending on-hardware check:** `lightshow off|on` from marvin behaves as before, and the dashboard dot follows the node rather than the command.
