...
**2026-08-11 (later) — the jaw animates itself; nothing on the bus commands it.** Greg's call, and the interesting half of it is the architecture: *let lemmy do his own thing with the jaw.* No opcode, no marvin state, no per-frame command stream — `beat_jaw.{c,h}` is a small pose machine fed from the beat frames lemmy already consumes. Poses, from Greg's hand-found positions: **relaxed open (`-50`)** where the jaw lives, **wide open (`-100`)** for a yell, **shut (`0`)** now and then. Held for **seconds**, not chewed once per beat — a per-beat chomp is both a different feature ("talking", still L4) and a good way to wear out a hobby servo. The **yell requires the neck to actually be driven** (a burst in auto, every frame in always) and hangs on a `BIG_BEAT`, held 1.5-3 s, no more often than every 6-14 s: it should read as him getting into it, which means it can only happen when he *is* into it. **Shutting his mouth needs no excuse** — 1-2.5 s, 12-28 s apart, and it happens between bursts too, which is deliberate: in `auto` the neck parks between bursts, and a puppet frozen mid-pose for 20 s looks broken, whereas a mouth that occasionally closes looks like something idling. Moves are **slew-limited** (7 counts/frame ≈ 160/s, so the full open-to-shut sweep is ~2/3 s) with the yell snapping open at 40/frame; a jaw that jumps between poses reads as a servo twitch rather than a jaw. **Gating reuses the nod mode rather than adding a knob:** the jaw animates whenever the mode is anything but off, and `nod off` parks it at rest and then leaves it alone — so the existing "`nod off` frees the servo for manual/`0x88B5` control" story now covers *both* servos, which it has to, since the `0x88B5` payload carries neck and jaw together and an animator writing every frame would stomp a manual jaw position. `beat_nod` owns the frame tick so it owns the jaw's clock (init, per-frame call, and park all go through it) — one frame-consumption point, explicit ordering, no second copy of "has a new frame arrived". Its own LFSR (seeded differently, stirred with the energy byte) so pose timing doesn't correlate with the nod's burst jitter. New `jaw:` line in the `nod` CLI status (pose, position, hold remaining, yell/close counts). 576 B of flash. Builds clean. **Pending on-hardware check:** the three poses look right and the slew rate is neither sluggish nor twitchy; a yell only ever lands during a head-bang; `nod off` leaves `pos jaw <n>` alone.
